...
(function () {
  ...
  function normalizeSessionDate(root) {
    ...
    var addAction = head.querySelector('.rb-pixel-add-action'), side = head.querySelector('.xm-fit-date-side'), oldMeta = root.querySelector('.xm-fit-session-meta');
    if (oldMeta) oldMeta.remove();
    if (!side) {
      side = document.createElement('div');
      side.className = 'xm-fit-date-side';
      side.setAttribute('aria-live', 'polite');
      if (addAction) head.insertBefore(side, addAction); else head.appendChild(side);
    }
    ...
    if (back && !back.getAttribute('data-xm-fit-bound')) {
      ...
      back.setAttribute('data-xm-fit-bound', '1');
    }
  }
  ...
  function normalizeFitNav(root) {
    ...
    qa('.xm-part-title', root).forEach(function (title) {
      title.classList.remove('xm-part-complete');
      qa('.xm-part-title-dot', title).forEach(function (dot) { dot.remove(); });
    });
  }
  ...
  function decorateActionResults() {
    ...
    Array.prototype.forEach.call(document.querySelectorAll('#rbFitResults .rb-food-result'), function (row, index) {
      var item = matches[index] || {}, guide = row.querySelector('.rb-result-guide'), onclick = guide && guide.getAttribute('onclick') || '', idMatch = onclick.match(/rbShowExerciseGuide\(['"]([^'"]+)/), src = idMatch ? exerciseImageForId(idMatch[1]) : '';
      if (!src && item.image) src = item.image;
      if (!src) src = actionImage(item);
      var art = row.querySelector('.xm-action-result-art'), img = art && art.querySelector('img');
      if (!art) { art = document.createElement('span'); art.className = 'xm-action-result-art'; row.insertBefore(art, row.firstChild); }
      if (!img) { img = document.createElement('img'); art.appendChild(img); }
      if (src) { img.src = src; img.alt = ''; }
    });
  }
  function exerciseUseCount(id, label) {
    var days = appData().days || {}, count = 0, key = String(id || ''), name = String(label || '').trim();
    Object.keys(days).forEach(function (dayKey) {
      var fit = (days[dayKey] || {}).fit || {};
      (fit.parts || []).forEach(function (part) {
        (part.items || []).forEach(function (item) {
          if (!item || item.deleted) return;
          var itemId = item.eid || item.exerciseId || item.exercise_id || '';
          if ((key && String(itemId) === key) || (name && String(item.name || '').trim() === name)) count += 1;
        });
      });
    });
    return count;
  }
  function sortActionResults() {
    var box = q('#rbFitResults');
    if (!box) return;
    var rows = qa('.rb-food-result', box), matches = window.rbFitMatches || [];
    var entries = rows.map(function (row, index) {
      var guide = q('.rb-result-guide', row), raw = guide && guide.getAttribute('onclick') || '';
      var idMatch = raw.match(/rbShowExerciseGuide\(['"]([^'"]+)/), title = q('b', row);
      return {row:row, index:index, id:idMatch ? idMatch[1] : '', label:title ? title.textContent : '', use:exerciseUseCount(idMatch ? idMatch[1] : '', title ? title.textContent : '')};
    });
    entries.sort(function (a, b) { return b.use - a.use || a.index - b.index; });
    entries.forEach(function (entry) { box.appendChild(entry.row); });
    if (Array.isArray(matches) && matches.length) {
      var sorted = matches.slice().sort(function (a, b) {
        return exerciseUseCount(b && b.id, b && b.name) - exerciseUseCount(a && a.id, a && a.name);
      });
      matches.splice.apply(matches, [0, matches.length].concat(sorted));
      entries.forEach(function (entry, index) { entry.row.setAttribute('onclick', 'rbPickExercise(' + index + ')'); });
    }
  }
  function wrapExerciseSearch() {
    var fn = window.rbSearchExercise; if (typeof fn !== 'function' || fn.__xmActionImage) return;
    var wrapped = function () { var result = fn.apply(this, arguments); setTimeout(function () { sortActionResults(); decorateActionResults(); }, 0); return result; };
  ...
  function wrapSheet() {
    ...
    if (typeof open === 'function' && !open.__xmFitSheet) { var openWrapped = function () { var result = open.apply(this, arguments); setTimeout(function () { sheetChrome(); document.body.classList.add('xm-fit-sheet-open'); sortActionResults(); decorateActionResults(); }, 0); return result; }; openWrapped.__xmFitSheet = true; window.rbOpenFitPart = openWrapped; }
  ...
  function normalizeFitCards(root) {
    var recHead = root.querySelector('.xm-rec-head'), planGrid = root.querySelector('.xm-plan-grid');
    if (recHead && planGrid && !recHead.closest('.xm-fit-rec-card')) {
      var recCard = document.createElement('section'); recCard.className = 'xm-fit-rec-card';
      recHead.parentNode.insertBefore(recCard, recHead); recCard.appendChild(recHead); recCard.appendChild(planGrid);
    }
    var sessionHead = root.querySelector('.xm-fit-session-head'), dayTabs = root.querySelector('.xm-daytabs-wrap, .xm-fit-daytabs'), session = root.querySelector('.rb-pixel-session');
    if (sessionHead && session && !sessionHead.closest('.xm-fit-session-card')) {
      var sessionCard = document.createElement('section'); sessionCard.className = 'xm-fit-session-card';
      sessionHead.parentNode.insertBefore(sessionCard, sessionHead); sessionCard.appendChild(sessionHead);
      if (dayTabs) sessionCard.appendChild(dayTabs);
      sessionCard.appendChild(session);
    }
  }
  function styles() {
    if (document.querySelector('#xm-fit-polish-style')) { layoutStyles(); finalLayoutStyles(); return; }
  ...
  function layoutStyles() {
    if (document.querySelector('#xm-fit-layout-v2')) return;
  ...
  function finalLayoutStyles() {
    if (document.querySelector('#xm-fit-layout-final')) return;
  ...
  function fitFinishStyles() {
    if (document.querySelector('#xm-fit-finish-style')) return;
    var style = document.createElement('style'); style.id = 'xm-fit-finish-style';
    style.textContent = '.xm-fit-rec-card,.xm-fit-session-card{box-sizing:border-box!important;margin:10px 0 12px!important;padding:8px!important;border:1px solid #d7c3a6!important;border-radius:17px!important;background:rgba(255,250,240,.96)!important;box-shadow:0 7px 16px rgba(86,64,33,.07)!important}.xm-fit-rec-card>.xm-rec-head{margin:0 0 7px!important;padding:0 1px!important}.xm-fit-rec-card>.xm-plan-grid{margin:0!important}.xm-fit-session-card{margin-top:8px!important;padding-top:8px!important}.xm-fit-session-card>.xm-fit-session-head{margin:0 0 5px!important}.xm-fit-session-card>.xm-daytabs-wrap,.xm-fit-session-card>.xm-fit-daytabs{margin:0 0 8px!important}.xm-fit-session-card>.rb-pixel-session{margin:0!important}.xm-fit-session-head>.rb-pixel-section-title{flex:0 1 auto!important;min-width:0!important;margin-right:0!important;white-space:nowrap!important}.xm-fit-date-side{display:inline-flex!important;align-items:center!important;justify-content:flex-start!important;gap:5px!important;flex:0 0 auto!important;min-width:0!important;margin-left:8px!important;white-space:nowrap!important;color:#846d56!important;font-size:10px!important;line-height:1!important}.xm-fit-date-side[hidden]{display:none!important}.xm-fit-session-date{display:inline-block!important;font-size:10px!important;font-weight:850!important;letter-spacing:.01em!important;white-space:nowrap!important}.xm-fit-date-side .xm-fit-back-today{display:inline-flex!important;align-items:center!important;justify-content:center!important;height:24px!important;min-height:24px!important;padding:0 8px!important;border:1px solid #b8c98e!important;border-radius:999px!important;background:#f1f5e5!important;color:#61782f!important;font:inherit!important;font-size:10px!important;font-weight:900!important;line-height:22px!important;white-space:nowrap!important;cursor:pointer!important;pointer-events:auto!important}.xm-fit-session-head>.rb-pixel-add-action{flex:0 0 auto!important;min-width:0!important;margin-left:auto!important;white-space:nowrap!important;pointer-events:auto!important}.xm-fit-session-head .xm-fit-date-side+ .rb-pixel-add-action{margin-left:auto!important}.xm-part-title::before,.xm-part-title::after,.xm-part-title-dot{content:none!important;display:none!important}.xm-part-title.xm-part-complete{color:inherit!important}#rbFitSheet:not([hidden]){display:flex!important;align-items:flex-end!important;justify-content:center!important}#rbFitSheet .rb-sheet-panel{display:flex!important;flex-direction:column!important;box-sizing:border-box!important;width:min(100%,430px)!important;max-height:min(84svh,680px)!important;overflow:hidden!important;margin:0!important;padding-bottom:calc(8px + env(safe-area-inset-bottom))!important}#rbFitSheet .rb-sheet-head,#rbFitSheet .rb-choice-row,#rbFitSheet #rbFitSearch{flex:0 0 auto!important}#rbFitSheet #rbFitResults{flex:1 1 auto!important;min-height:0!important;max-height:none!important;overflow:auto!important;overscroll-behavior:contain!important}#rbFitSheet .rb-sheet-actions{position:sticky!important;bottom:0!important;z-index:10!important;flex:0 0 auto!important;margin:8px 0 0!important;padding:8px 0 calc(4px + env(safe-area-inset-bottom))!important;background:linear-gradient(180deg,rgba(255,253,248,.82),#fffdf8 30%)!important}@media(max-width:699px){body.rb-mode .rebuild-app,body.rb-mode #rbScreen{width:100%!important;max-width:none!important;margin-left:0!important;margin-right:0!important}body.rb-mode #rbNav{width:calc(100% - 24px)!important;max-width:none!important}}#rbFitSheet .rb-sheet-panel{display:grid!important;grid-template-columns:66px minmax(0,1fr)!important;grid-template-rows:auto auto auto auto!important;align-content:start!important;overflow-y:auto!important;overflow-x:hidden!important;scrollbar-width:none!important}#rbFitSheet .rb-sheet-panel::-webkit-scrollbar,#rbFitSheet #rbFitResults::-webkit-scrollbar,#rbFitSheet #rbFitGroups::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}#rbFitSheet .rb-sheet-head{grid-column:1/-1!important;grid-row:1!important}#rbFitSheet #rbFitGroups{grid-column:1!important;grid-row:2/4!important;display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:6px!important;width:58px!important;max-height:none!important;overflow:visible!important;padding:0!important;position:sticky!important;top:0!important;align-self:start!important}#rbFitSheet #rbFitGroups .rb-choice{box-sizing:border-box!important;width:58px!important;min-height:36px!important;height:36px!important;flex:0 0 36px!important;padding:0 4px!important;border-radius:11px!important;white-space:nowrap!important;font-size:11px!important}#rbFitSheet #rbFitSearch{grid-column:2!important;grid-row:2!important;min-width:0!important;margin:0 0 8px!important}#rbFitSheet #rbFitResults{grid-column:2!important;grid-row:3!important;display:flex!important;flex-direction:column!important;min-width:0!important;min-height:0!important;max-height:none!important;overflow:visible!important;overscroll-behavior:initial!important;gap:8px!important}#rbFitSheet .rb-sheet-actions{grid-column:1/-1!important;grid-row:4!important}@media(max-width:420px){#rbFitSheet .rb-sheet-panel{grid-template-columns:62px minmax(0,1fr)!important}#rbFitSheet #rbFitGroups,#rbFitSheet #rbFitGroups .rb-choice{width:54px!important}}#rbFitResults .rb-food-result>.xm-action-result-art{display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;width:48px!important;max-width:48px!important;height:48px!important;flex:0 0 48px!important;margin-right:8px!important}#rbFitResults .rb-food-result>.xm-action-result-art img{width:42px!important;height:42px!important;object-fit:contain!important}body.xm-fit-sheet-open{overflow:hidden!important}';
    document.head.appendChild(style);
  }
  function refresh() { styles(); fitFinishStyles(); wrapExerciseSearch(); wrapSheet(); var root = cardRoot(); if (root) { normalizeFitNav(root); normalizeFitCards(root); sortActionResults(); decorateActionResults(); renderAnalysis(); } }
