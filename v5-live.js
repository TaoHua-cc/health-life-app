...
(function () {
  ...
  function refreshHome() {
    ...
    if(foot[1]&&hasFit){var mt=q('b',foot[1]);if(mt)mt.textContent=Math.round(m.min)+' '+TEXT.min;}
    var week=weekData(), complete=week.filter(function(v){return v>=goal();}).length; if(foot[2] && hasFit){var gt=q('b',foot[2]);if(gt)gt.textContent=complete+' / 4 天';}
    if(hasMeals){var keys=['breakfast','lunch','dinner'],imgs=['home-breakfast-bowl.png','home-lunch-salmon.png','home-dinner-soup.png'],labels=['早餐','午餐','晚餐'],mealCards=qa('.rb-home-pixel-meal',root);mealCards.forEach(function(card,i){var rows=(d.meals&&d.meals[keys[i]])||[],total=rows.reduce(function(sum,x){return sum+n(x.cal||x.kcal||x.calories);},0),title=q('b',card),sub=q('small',card);if(title)title.textContent=labels[i];if(sub)sub.textContent=rows.length?Math.round(total)+' kcal':'未记录';var im=q('img',card);if(im){im.src='assets/xiaoman-v3/'+imgs[i];im.alt=labels[i];}});}
