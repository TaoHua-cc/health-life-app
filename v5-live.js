...
(function () {
  ...
  function refreshHome() {
    ...
    if (records) { var ring=q('.rb-home-pixel-ring div',root); if(ring) ring.innerHTML='<b>'+Math.round(eaten)+'</b><span>/'+goalK+' kcal</span><small>\u76ee\u6807 '+goalK+' kcal</small>'; }
    ...
    if (records) qa('.rb-home-pixel-macros > div',root).forEach(function(row,i){var k=['carb','protein','fat'][i],b=row.querySelector('b'),em=row.querySelector('em');if(b)b.textContent=Math.round(macro[k])+' g';if(em)em.style.width=Math.min(100,Math.round(macro[k]/macroMax[k]*100))+'%';});
    ...
    if(foot[1]&&records){var mt=q('b',foot[1]);if(mt)mt.textContent=Math.round(m.min)+' '+TEXT.min;}
    var week=weekData(), complete=week.filter(function(v){return v>=goal();}).length; if(foot[2] && records){var gt=q('b',foot[2]);if(gt)gt.textContent=complete+' / 4 天';}
    if(records){var keys=['breakfast','lunch','dinner'],imgs=['home-breakfast-bowl.png','home-lunch-salmon.png','home-dinner-soup.png'],labels=['早餐','午餐','晚餐'],mealCards=qa('.rb-home-pixel-meal',root);mealCards.forEach(function(card,i){var rows=(d.meals&&d.meals[keys[i]])||[],total=rows.reduce(function(sum,x){return sum+n(x.cal||x.kcal||x.calories);},0),title=q('b',card),sub=q('small',card);if(title)title.textContent=labels[i];if(sub)sub.textContent=rows.length?Math.round(total)+' kcal':'未记录';var im=q('img',card);if(im){im.src='assets/xiaoman-v3/'+imgs[i];im.alt=labels[i];}});}
