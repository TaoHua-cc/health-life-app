...
(function(){
  ...
  document.addEventListener('click',function(e){var add=e.target.closest&&e.target.closest('.rb-ref-food-add,.rb-ref-meal-add-inline');if(add){e.preventDefault();e.stopPropagation();var inline=(add.getAttribute('onclick')||'').match(/\(['\"](breakfast|lunch|dinner|snack)['\"]\)/),key=inline&&inline[1]||'breakfast';if(typeof window.rbOpenMealAddFromPlate==='function')window.rbOpenMealAddFromPlate(key);return}var sync=e.target.closest&&e.target.closest('.rb-ref-sync');if(sync){e.preventDefault();if(typeof window.rbOpenNativeHealth==='function')window.rbOpenNativeHealth();return}var row=e.target.closest&&e.target.closest('.rb-ref-food-row');if(row){openSheet(row.getAttribute('data-rb-food-meal'),Number(row.getAttribute('data-rb-food-index')));return}if(e.target.closest&&e.target.closest('.rb-food-edit-close,.rb-food-edit-cancel')){closeSheet();return}if(e.target.closest&&e.target.closest('.rb-food-edit-save')){apply();return}if(e.target.closest&&e.target.closest('.rb-food-edit-delete'))remove()});
